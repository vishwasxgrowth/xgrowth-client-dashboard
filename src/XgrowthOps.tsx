...
function DashboardTab({ R, range, setRange, cs, setCs, ce, setCe, q, sortKey, setSortKey, sortDir, setSortDir, threshold, openApp }) {
  const kpis = useMemo(() => {
    ...
    return [
      ...
      { label: "ARPDAU", value: money4(cur.arpdau), c: cur.arpdau, p: prv.arpdau, series: sparkDaily.map((d) => d.arpdau) },
    ].map((k) => ({ ...k, d: delta(k.c, k.p), spark: sparkPath(k.series, 112, 30) }));
  }, [R]);
  const COLS = [{ k: "name", label: "App" }, { k: "revenue", label: "Revenue" }, { k: "ecpm", label: "eCPM" }, { k: "arpdau", label: "ARPDAU" }, { k: "impressions", label: "Impressions" }, { k: "matchRate", label: "Match rate" }, { k: "ctr", label: "CTR" }, { k: "showRate", label: "Show rate" }];
  const { rows, alerts } = useMemo(() => {
    ...
    let al = 0;
    const rws = list.map(({ app, a, b }) => { const dr = delta(a.revenue, b.revenue); if (dr.v < -threshold) al++; const cells = [{ v: money(a.revenue), d: dr }, { v: money2(a.ecpm), d: delta(a.ecpm, b.ecpm) }, { v: money4(a.arpdau), d: delta(a.arpdau, b.arpdau) }, { v: compact(a.impressions), d: delta(a.impressions, b.impressions) }, { v: pct(a.matchRate), d: delta(a.matchRate, b.matchRate) }, { v: pct(a.ctr), d: delta(a.ctr, b.ctr) }, { v: pct(a.showRate), d: delta(a.showRate, b.showRate) }]; return { id: app.id, name: app.name, meta: app.cat + " \u00b7 " + app.tier, initials: appInitials(app.name), color: appColor(app.id), cells }; });
    return { rows: rws, alerts: al };
  }, [R, q, sortKey, sortDir, threshold]);
  const gridCols = "minmax(210px,1.6fr) repeat(7,1fr)";
...
function AppsTab({ R, q, appId, setAppId, appTab, setAppTab, chartDays, setChartDays, hov, setHov, tasks, taskView, openTest }) {
  ...
  const stats = [{ label: "Revenue", v: money(a.revenue), d: delta(a.revenue, b.revenue) }, { label: "eCPM", v: money2(a.ecpm), d: delta(a.ecpm, b.ecpm) }, { label: "ARPDAU", v: money4(a.arpdau), d: delta(a.arpdau, b.arpdau) }, { label: "Impressions", v: compact(a.impressions), d: delta(a.impressions, b.impressions) }, { label: "Match rate", v: pct(a.matchRate), d: delta(a.matchRate, b.matchRate) }];
